...
이 문서는 `user-service` 구현 코드를 수정한 뒤 계약 변경을 어떻게 처리할지 정리한다. 기준 계약의 원본은 `https://github.com/jho951/service-contract`이고, 이 repo는 `contract.lock.yml`로 특정 contract commit을 따른다.
...
`service-contract`, `docs/openapi/user-service.yml`, `contract.lock.yml`을 건드리지 않는다.
...
git commit -m "Update user-service implementation"
...
git commit -m "Update user-service contract"
...
```bash
cp /Users/jhons/Downloads/BE/contract/service-contract/artifacts/openapi/user-service.v1.yaml \
  docs/openapi/user-service.yml
```

- contract 원본: `artifacts/openapi/user-service.v1.yaml`
- service local copy: `docs/openapi/user-service.yml`
...
git commit -m "Align user-service implementation with contract"
git push origin <branch>
```
...
| 변경 내용 | 올릴 위치 |
| --- | --- |
| API path, request, response, status, auth 변경 | `service-contract/artifacts/openapi`, `service-contract/repositories/user-service` |
| 공통 header, error, security, env, CI/CD 규칙 변경 | `service-contract/shared` |
| 운영 port, health, ready, compose, monitoring target 변경 | `service-contract/repositories/user-service/ops.md` 또는 관련 운영 문서 |
